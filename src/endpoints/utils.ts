...
export function response(result: Record<string, unknown> | Array<unknown>, confidence: Confidence, warnings: string[] = [], supported = true, evidence?: any, next_action?: string) {
  return {
    supported,
    result,
    confidence,
    ...
    for (const [key, prop] of Object.entries(schema.properties) as any) {
      if (key in body && body[key] !== null && body[key] !== undefined && body[key] !== "") {
        ...
        if (prop.type === "string") {
          ...
          if ((key === "address" || key === "wallet") && !/^0x[a-fA-F0-9]{40}$/.test(strVal)) {
